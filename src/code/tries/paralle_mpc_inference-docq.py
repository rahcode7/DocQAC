...
def common_queries(a,b):
    ...
    ab_dict = {}
    common_ab = a_set.intersection(b_set)
    for q in common_ab:
        if q in a.keys() and q in ab_dict.keys():
            ab_dict[q] += a[q] 
        else:
            ab_dict[q] =  a[q] 
        if q in b.keys() and q in ab_dict.keys():
            ab_dict[q] += b[q] 
        else:
            ab_dict[q] =  b[q] 
...
def get_completions(main_trie,suffix_trie=None,prefix=None,k_completions=10, suffix_context=2):
    res = []
    ...
    #################################### Docquery completions
    #print(prefix)
   # docquery_completions = docquery_trie.find_completions(str(prefix))
    #prefix = str(prefix)
    #print(prefix)
    ...
    for z in main_completions:
        ...
        main_completions_dict[z[0]]=1
        # adding prefix "MT" to denote main trie completions
        res.append([z[0], "DQT:%s" % str(z[1])])

    #################################### Docquery completions
    #print(prefix)
    # doccontent_trie_completions = doccontent_trie.find_completions(str(prefix))

    # main_completions_dict = {}
    # for z in docquery_completions:
    #     if z[0] in main_completions_dict:
    #         continue
    #     main_completions_dict[z[0]]=1
    #     # adding prefix "MT" to denote main trie completions
    #     res.append([z[0], "DQT:%s" % str(\[1])])



    #doccontent_trie_completions = doccontent_trie.find_completions(prefix)
    
    # Get overlap of these 3 prefix tries
   # a,b,c = main_completions,docquery_completions,doccontent_trie_completions



    # d_ab = common_queries(a,b)
    # d_ac = common_queries(a,c)


    # Priority 1 main and docquery
    # N = k_completions
    # if len(d_ab)>=N:
    #     res.append(dict(sorted(d_ab.items(),key=lambda x:x[1],reverse=True))[:N])
    # else:
    #     res.append(dict(sorted(d_ab.items(),key=lambda x:x[1],reverse=True)))

    # # P2 main and docontent
    # remaining = k_completions-len(res)
    # if len(res)< remaining and len(d_ac)>=remaining:  # if not found
    #     res.append(dict(sorted(d_ab.items(),key=lambda x:x[1],reverse=True))[:remaining])
    # else:
    #     res.append(dict(sorted(d_ab.items(),key=lambda x:x[1],reverse=True)))
...
def inference(args):
    ...
    for status, query,docid,prefix_data in stream: 
        ...
        # prefix = "justice"
        #if docid:
        doctrie_path = args.main_trie + '/' + str(docid)  + '.mpc'
        #print(doctrie_path)

        st = time.time()   
        query_completion = load_trie_from_file(doctrie_path)
        et = time.time()
        load_time += et-st

        doctrie_suffix_path =  args.main_trie + "/suffix/" + str(docid)  + '.mpc'

        st = time.time()   
        suffix_completion = load_trie_from_file(doctrie_suffix_path)
        et = time.time()
        load_time += et-st
    ...
    f = open( 'docquery-tries/docquery-trie-disk.txt', 'a' )
