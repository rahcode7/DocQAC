...
def inference(args):
    ...
    for status, query,docid,prefix_data in stream: 
        ...
        #print(f'Query Completion {qusery_completion}')
        ...
        # f = open( 'doc-tries/doc-trie-disk.txt', 'a' )
        # f.write( 'File : ' + args.input_file + ' Total Disk load time (Milliseconds): ' + str(load_time) + '\n' )
        # f.close()
        # take last 50 characters within the prefix
        # prefix = prefix[-50:]
        completions = get_completions(query_completion, suffix_completion, prefix, 
                                    args.k_completions, 2)
