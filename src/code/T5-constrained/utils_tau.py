        ...
        if in_type != "no_doc":
            ...
            if self.in_type == "yake":
                ...
                try:
                    ...
                    self.doc["yake"] = self.doc["yake"].apply(lambda x: self.trunc_text(x, 352)["text"])
            ...
            if self.in_type == "full_doc":
                self.doc["url"] = self.doc["doc_url"].apply(lambda x: self.trunc_text(x, 32)["text"])
                self.doc["body"] = self.doc["body"].apply(lambda x: self.trunc_text(x, 384)["text"])
                self.doc["heading"] = self.doc["heading"].apply(lambda x: " ".join(x.split(" ")[:32]))
                # self.
            ...
            if self.in_type == "summary":
                #print("*"*100)
                self.doc["url"] = self.doc["doc_url"].apply(lambda x: self.trunc_text(x, 32)["text"])
                self.doc["body"] = self.doc["body"].apply(lambda x: self.trunc_text(x, 352)["text"])
                self.doc["heading"] = self.doc["heading"].apply(lambda x: " ".join(x.split(" ")[:32]))
            
            if self.in_type == "url":
                self.doc["body"] = self.doc["doc_url"].apply(lambda x: self.trunc_text(x, 384)["text"])
                self.doc["heading"] = self.doc["heading"].apply(lambda x: " ".join(x.split(" ")[:32]))
    ...
    def __getitem__(self, idx):
        ...
        if(self.infer):
            if self.in_type in ["full_doc", "summary"]:
                ...
                url = curr_eg["url"]
                input = "title: " + str(heading) + " url: " + str(url) + " context: " + str(ctx) + " query: " + str(curr_eg["prefix"])
                self.max_length = 512
            
            elif self.in_type == "url":
                ctx = curr_eg["body"]
                heading = curr_eg["heading"]
                input = "title: " + str(heading) + " url: " + str(ctx) + " query: " + str(curr_eg["prefix"])
                self.max_length = 192

            elif self.in_type == "no_doc":
                # print(curr_eg)
                input = str(curr_eg["prefix"])
                ...
                budget = (352 - chunks_len)//chunks_len
                final_cated = ""
                for c in chunks:
                    # print(c)
                ...
                input = "title: " + str(curr_eg["heading"]) + " url: " + str(curr_eg["url"]) + " context: " + str(final_cated) + " query: " + str(curr_eg["prefix"])
                self.max_length = 512
        ...
        if self.in_type in ["full_doc", "summary"]:
            # print(curr_eg.keys())
            input_text = "title: " + str(curr_eg["heading"]) + " url: " + str(curr_eg["url"]) + " context: " + str(curr_eg["body"]) + " query: " + str(input_text)
            self.max_length = 512
        
        elif self.in_type == "url":
            input_text = "title: " + str(curr_eg["heading"]) + " url: " + str(curr_eg["body"]) + " query: " + str(input_text)
            self.max_length = 192
            ...
            input_text = "title: " + str(curr_eg["heading"]) + " url: " + str(curr_eg["url"]) + " context: " + str(curr_eg["yake"]) + " query: " + str(input_text)
            self.max_length = 512
            ...
            # remove last <eok>
            final_cated = final_cated[:-7]
            input_text = "title: " + str(curr_eg["heading"]) + " url: " + str(curr_eg["url"]) + " context: " + str(final_cated) + " query: " + str(input_text)
            self.max_length = 512
