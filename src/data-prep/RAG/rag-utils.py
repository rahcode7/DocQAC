...
def get_chunks_dense(prefix,docid,text,embeddings,text_splitter,k=20):
    ...
    f"datasets/rag/vector_stores/test/{docid}", embeddings, allow_dangerous_deserialization=True 
    )
    ...
    for prefix_word in prefix_list:
        ...
        rel_docs = []
        rel_chunks = []
        filtered_docs = filter_docs_by_prefix(prefix_word, all_splits_docs)
...
def get_chunks_similar_dense(prefix,docid,text,sim_docs,embeddings,dataset,text_splitter,k=40):
    prefix = str(prefix)
    all_rel_chunks = [] 
    ...
    for docid in sim_docs:
        ...
        doc_vector_store = FAISS.load_local(
        f"datasets/rag/vector_stores/{dataset}/{docid}", embeddings, allow_dangerous_deserialization=True 
        )
